...
#inputs
CLINICAL_DATA_DF_FILE = "/neurospin/rlink/REF_DATABASE/rawdata/participants.tsv"
...
def save_df_ROI(verbose=True, longitudinal=False, save=False):
    """
    ...
    if save:
        # save the df
        if longitudinal: filename= "df_ROI_age_sex_site_M00_M03.csv"
        else : filename = "df_ROI_age_sex_site_M00.csv"
        df_ROI_age_sex_site.to_csv(ROOT+filename, index=False)
        print("df saved to : ",filename)


def main():
    save_df_ROI(longitudinal=True) 
