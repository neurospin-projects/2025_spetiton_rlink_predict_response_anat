...
config['prefix'] = "40_randomize_feature_importance"
config['input_permutations'] = "40_randomize_feature_importance_seeds.csv"
 

# Set output paths
config['output_rois'] = \
    os.path.join(config['output_models'], config['prefix'] + "_rois.csv")
config['output_randomize_feature_importance'] = \
    os.path.join(config['output_models'], config['prefix'] + ".xlsx")
...
print(predictions_metrics_df[predictions_metrics_df.perm == 'perm-000'])
"""
                                        model      perm  balanced_accuracy   roc_auc
0     model-grpRoiLda+lrl2_resid-age+sex+site  perm-000           0.703175  0.690185
1001          model-lrl2cv_resid-age+sex+site  perm-000           0.691230  0.683968
"""
...
with pd.ExcelWriter(config['output_randomize_feature_importance']) as writer:#, mode="a", if_sheet_exists="replace") as writer:
