...
from nitk.ml_utils.sklearn_utils import pipeline_behead, get_linear_coefficients
...
csf_indices = np.array([i for i, col in enumerate(input_columns) if 'CSF' in col])
X[:, (csf_indices)] *= -1
...
assert Z_ncol + len(input_columns) == X.shape[1]  # Check that the number of columns is correct

# Check that X == [Z + X[input_columns]]
X_ = get_X(data, input_columns, print_log=print).values
X_[:, (csf_indices)] *= -1
...
def group_by_roi(input_columns):
    ...
    Args:
        input_columns (list): List of column names to group.
    Returns:
    ...
    Exemple:
        ...
        ... ]
        >>> result = group_by_roi(input_columns_)
        >>> print(result)
...
roi_groups = group_by_roi(input_columns)
roi_groups = {roi:[input_columns.index(x) for x in cols] for roi, cols in roi_groups.items()}

pd.DataFrame([[roi, len(subrois), ",".join(subrois)] for roi, subrois in group_by_roi(input_columns).items()],
            columns=['ROI', 'N_features', 'Features']).to_csv(config['prefix'] + "_roi_groups.csv", index=False)

assert len(input_columns) == 268
assert len(roi_groups) == 71
mask_gm = np.array(['GM' in s for s in input_columns])
mask_csf = np.array(['CSF' in s for s in input_columns])
mask_left = np.array(['Left' in s for s in input_columns])
mask_right = np.array(['Right' in s for s in input_columns])
...
np.sum(['GM' in s for s in input_columns]) == 134
np.sum(['CSF' in s for s in input_columns]) == 135
np.sum(['Left' in s for s in input_columns]) == 126
np.sum(['Right' in s for s in input_columns]) == 126
...
if 'output_predictions_scores_feature-importance' in config:
    ...
    features_names = {
        'model-lrl2_resid-age+sex+site':input_columns,
        'model-mlp_resid-age+sex+site':input_columns,
    ...
    for (mod, stat), df in features_stats.groupby(['model', 'stat']):
        ...
        pval_tmax_fwer = (stat_max_rnd > stat_values[:, None]).sum(axis=1) / nperm # pval_tmax_fwer.shape (nfeatures, )
...
feats_selected = [input_columns[feat_idx] for roi in rois_selected for feat_idx in roi_groups[roi]]
#[[roi, input_columns[feat_idx]] for roi in rois_selected for feat_idx in roi_groups[roi]]
...
for (mod, perm, fold), (estimator, X, y, train_idx, test_idx) in models_cv.items():
    ...
    len(input_columns)
    forward_corr_ = pd.DataFrame(dict(model=mod, perm=perm, fold=fold, feature=input_columns, Xy_corr=corr_.ravel()))
    ...
    for roi, transformer, columns in transformers:
        ...
        for i, col in enumerate(columns):
            transformers_coefs.append([mod, perm, fold, roi, input_columns[col], transformer.coef_.ravel()[i]])
...
X_train_res_df = pd.DataFrame(X_train_res, columns=input_columns)
...
#.style.background_gradient(cmap='coolwarm', axis=None, vmin=-1, vmax=1).set_precision(2).set_caption('Correlation matrix of residualized data')
...
if False:
    ...
    input_columns_nocsf = [col for col in input_columns if not 'CSF' in col]


    # Check that X == [Z + X[input_columns]]
    X_nocsf = get_X(data, input_columns_nocsf, print_log=print).values
    #X_[:, (csf_indices)] *= -1
    X_nocsf = residualizer_estimator.pack(Z, X_nocsf)
    assert X_nocsf.shape[1] == Z.shape[1] + len(input_columns_nocsf)
    ...
    roi_groups_nocsf = group_by_roi(input_columns_nocsf)
    roi_groups_nocsf = {roi:[input_columns_nocsf.index(x) for x in cols] for roi, cols in roi_groups_nocsf.items()}
