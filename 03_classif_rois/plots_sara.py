"""
scp triscotte.intra.cea.fr:/neurospin/signatures/2025_spetiton_rlink_predict_response_anat_backup/python_scripts/plots.py ./
"""
import nibabel
...
VOL_FILE_VBM = "./data/labels_Neuromorphometrics.nii"
DATA_DIR = ROOT+"data/"


def plot_glassbrain(dict_plot=None, title="", list_negative=None, neuromorphometrics_roi_names=True, threshold=0): 
    """
    ...
    for name, val in dict_plot.items():
        ...
        if list_negative: # for CSF ROI, for which we wanna plot the opposite of the value to compare with GM volume changes
    ...
    if vmin >= 0:
        ...
        red_from_coolwarm = plt.cm.coolwarm(1.0)
        cmap = LinearSegmentedColormap.from_list("white_to_red", ["white", red_from_coolwarm])
        print("All values positives: cmap = LinearSegmentedColormap.from_list", "white_to_red", ["white", red_from_coolwarm])
    elif vmax <= 0:
        # if all values are negative, the color map should be a gradient from blue (min value) to white (0)
        blue_from_coolwarm = plt.cm.coolwarm(0.0)
        cmap = LinearSegmentedColormap.from_list("blue_to_white", [blue_from_coolwarm, "white"])
        print("All values negatives: cmap = LinearSegmentedColormap.from_list", "blue_to_white", [blue_from_coolwarm, "white"])
...
def main():
    RLINK_DATAFRAME_M00_M03 = ROOT+"data/processed/df_ROI_age_sex_site_M00_M03_v4labels.csv"
    df_M0 = pd.read_csv(RLINK_DATAFRAME_M00_M03) 
    col_names = list(df_M0.columns)
    amygdala_regions_names = [r for r in col_names if "Amy" in r and r.endswith("_GM_Vol")]
    hippo_regions_names = [r for r in col_names if "Hip" in r and r.endswith("_GM_Vol")]

    print(amygdala_regions_names,"\n",hippo_regions_names)
    dict_fwd_maps = {amygdala_regions_names[0]:0.034, amygdala_regions_names[1]:0.034,hippo_regions_names[0]:0.045 ,hippo_regions_names[1]:0.045 }
    plot_glassbrain(dict_plot=dict_fwd_maps)
