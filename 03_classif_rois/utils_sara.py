...
def stratified_split(df_input, test_size=0.2, random_state=42, verbose: bool = True, \
                     min_site_size: int = 70, q_age: int = 10):
...
def get_pair_dict_CSF_GM(list_rois=get_rois(WM=False)):
    """
    ...
    for idx, name in enumerate(list_rois):
...
def stratified_split_balanced_age(
    df_input, 
    test_size=0.2, 
    random_state=42, 
    verbose: bool = True, 
    min_site_size: int = 600, 
    q_age: int = 10, # how many bins the age column is divided into for stratification
    include_site=True,
    target_bin_size=9000,
    return_age_bins=False
):
    """
    Stratified split with controlled undersampling of age bins.
    target_bin_size: int, maximum number of samples per age bin. 
                     If None, uses median bin size.
    """
    df = df_input.copy() # 60341 subjects
    df['age_bin'] = pd.qcut(df['age'], q=q_age, duplicates='drop')
    ...
    if include_site: 
        ...
        rare_sites = site_counts[site_counts < min_site_size].index
        df['site_binned'] = df['site'].replace(rare_sites, 'other')
        # print("Samples per site_binned:", df['site_binned'].value_counts().to_dict())
        df['strata'] = (
            df['age_bin'].astype(str) + "_" +
            df['sex'].astype(str) + "_" +
            df['site_binned'].astype(str)
    ...
    strata_counts_sorted = strata_counts.sort_values()  # ascending by default
    if verbose: 
        print("Strata counts (smallest to largest):")
        print(strata_counts_sorted)
    
    # Determine the target size per age bin
    if target_bin_size is None:
        target_bin_size = int(df['age_bin'].value_counts().median())
    
    # Undersample each age_bin to at most target_bin_size
    df_balanced = df.groupby('age_bin', group_keys=False).apply(
        lambda x: x.sample(min(target_bin_size, len(x)), random_state=random_state)
    )
    ...
    train_idx, test_idx = train_test_split(
        df_balanced.index,
        test_size=test_size,
        stratify=df_balanced['strata'],
        random_state=random_state
    )
    
    df_train = df_balanced.loc[train_idx].drop(columns=['age_bin', 'strata'])
    df_test = df_balanced.loc[test_idx].drop(columns=['age_bin', 'strata'])
    
    if verbose:
        print("Balanced dataset:")
        print("Train size:", len(train_idx), "Test size:", len(test_idx))
        print("Samples per age_bin:", df_balanced['age_bin'].value_counts().to_dict())
    
    if return_age_bins: 
        return df_train, df_test, train_idx, test_idx, df['age_bin']

    else: return df_train, df_test, train_idx, test_idx
...
def main():
    ...
    quit()
    for i in range(5):
        dat = read_pkl("/neurospin/signatures/2025_spetiton_rlink_predict_response_anat_backup/reports/transfer_bdvshc/densenet121_vbm_bipolar/Test_"+str(i)+"_densenet121_vbm_bipolar_epoch199.pkl")
        print(dat["metrics"])
    quit()
